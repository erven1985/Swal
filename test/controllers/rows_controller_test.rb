...
class RowsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create row" do
    assert_difference('Row.count') do
      post rows_url, params: { row: { description: @row.description, event_date: @row.event_date } }
    end
  ...
  test "should update row" do
    patch row_url(@row), params: { row: { description: @row.description, event_date: @row.event_date } }
